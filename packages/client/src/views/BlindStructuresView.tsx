...
interface BlindLevel {
  is_break: boolean | number; small_blind: number; big_blind: number;
  duration_seconds: number; break_label?: string | null;
}

type EditableLevel = {
  is_break: boolean; small_blind: string; big_blind: string;
  duration_minutes: string; break_label: string;
};

function newLevel(): EditableLevel {
  return { is_break: false, small_blind: '25', big_blind: '50', duration_minutes: '15', break_label: '' };
}

function newBreak(): EditableLevel {
  return { is_break: true, small_blind: '0', big_blind: '0', duration_minutes: '10', break_label: 'Break' };
}
...
export default function BlindStructuresView() {
  ...
  const refresh = useCallback(async () => {
    setStructures(await api.getBlindStructures() as BlindStructure[]);
  }, []);
  ...
  async function loadStructure(structureId: number) {
    ...
    if (!structure) return;
    const rows = await api.getBlindStructureLevels(structureId) as BlindLevel[];
    setSelectedId(structureId);
    setName(structure.name);
    setLevels(rows.map((row) => ({
  ...
  function startNew() {
    setSelectedId(null); setName(''); setLevels([newLevel()]); setStatus(null);
  }

  async function save() {
    if (!name.trim()) { setStatus({ text: 'Blind structure name is required.', ok: false }); return; }
    if (levels.length === 0) { setStatus({ text: 'Add at least one level.', ok: false }); return; }

    const normalizedName = name.trim().toLowerCase();
    const duplicate = structures.find((item) => item.name.trim().toLowerCase() === normalizedName && item.id !== selectedId);
    if (duplicate) { setStatus({ text: `A blind structure named "${duplicate.name}" already exists.`, ok: false }); return; }

    const invalidLevel = levels.find((row) => {
      if (row.is_break) return false;
      const small = Number(row.small_blind); const big = Number(row.big_blind); const mins = Number(row.duration_minutes);
      return small <= 0 || big <= 0 || big < small || mins <= 0;
    });
    if (invalidLevel) { setStatus({ text: 'Each blind level needs positive Small/Big, Big >= Small, and positive minutes.', ok: false }); return; }
    ...
    try {
      ...
      if (selectedId === null) {
        const created = await api.createBlindStructure(payload) as BlindStructure;
        setSelectedId(created.id);
        setStatus({ text: `Created structure "${created.name}".`, ok: true });
      } else {
        const updated = await api.updateBlindStructure(selectedId, payload) as BlindStructure;
  ...
  async function remove() {
    ...
    setSaving(true); setStatus(null);
    try {
      await api.deleteBlindStructure(selectedId);
      ...
      <aside className="xl:col-span-1 space-y-3">
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-900/50 divide-y divide-gray-800">
          {structures.length === 0 ? (
            ...
            <button key={structure.id} type="button" onClick={() => { void loadStructure(structure.id); }}
      ...
      <section className="xl:col-span-2 space-y-4 rounded-xl border border-gray-800 bg-gray-900/40 p-5">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. OPT Deepstack"
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button type="button" onClick={() => setLevels((prev) => [...prev, newLevel()])}
              className="rounded-md border border-gray-700 px-3 py-1.5 text-xs font-semibold text-gray-200 hover:border-orange-400 hover:text-orange-300">
              + Level
            </button>
            <button type="button" onClick={() => setLevels((prev) => [...prev, newBreak()])}
          ...
          {levels.map((row, idx) => (
            <div key={idx} className="grid grid-cols-12 gap-2 rounded-lg border border-gray-800 bg-gray-950/40 p-2 items-center">
              ...
              <div className="col-span-2">
                ...
                <input disabled={row.is_break} value={row.small_blind}
              ...
              <div className="col-span-2">
                ...
                <input disabled={row.is_break} value={row.big_blind}
              ...
              <div className="col-span-2">
                ...
                <input value={row.duration_minutes}
              ...
              <div className="col-span-2">
                ...
                <input disabled={!row.is_break} value={row.break_label}
                  onChange={(e) => setLevels((prev) => prev.map((entry, rowIdx) => rowIdx !== idx ? entry : { ...entry, break_label: e.target.value }))}
                  className="w-full bg-gray-800 border border-gray-700 rounded px-2 py-1 text-xs disabled:opacity-40" />
              </div>
              <div className="col-span-12 flex justify-end">
                <button type="button" onClick={() => setLevels((prev) => prev.filter((_, rowIdx) => rowIdx !== idx))}
