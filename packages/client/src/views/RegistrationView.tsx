...
interface TableAssignment {
  id: number; table_id: number; table_name: string;
  player_name?: string | null; seat_number?: number | null;
}
...
export default function RegistrationView() {
  ...
  const refreshTournaments = useCallback(async () => {
    try {
      const all = await api.getAllTournaments() as Tournament[];
      setTournaments(all.filter((t) => t.status === 'pending'));
    } catch { /* ignore */ }
  }, []);
  ...
  const refreshPlayers = useCallback(async () => {
    if (!tournamentId) {
      setPlayers([]);
      setAssignments([]);
      try { setRoster(await api.getAllPlayers() as Player[]); } catch { /* ignore */ }
      return;
    }
    try {
  ...
  useEffect(() => { void refreshTournaments(); }, [refreshTournaments]);
  ...
  useEffect(() => { void refreshPlayers(); }, [refreshPlayers]);

  useTournamentProgressReset(useCallback(() => { void refreshPlayers(); }, [refreshPlayers]));
  ...
  async function registerPlayers(playerList: Player[]) {
    ...
    try {
      for (const player of playerList) {
        await api.registerPlayer({ name: player.name, tournamentId, chipCount: 10000 });
      }
      setSelectedIds([]); setSearch('');
      setStatus({
        text: playerList.length === 1
          ? `✓ ${playerList[0].name} registered!`
          : `✓ ${playerList.length} players registered!`,
        ok: true,
      });
      await refreshPlayers();
    } catch (err) { setStatus({ text: `Error: ${String(err)}`, ok: false }); }
    finally { setSubmitting(false); }
  }

  function toggleSelected(playerId: number) {
    setSelectedIds((cur) => cur.includes(playerId) ? cur.filter((id) => id !== playerId) : [...cur, playerId]);
  }
  ...
  async function handleAssignSeats() {
    ...
    setAssigning(true);
    try {
      const result = await api.randomAssignSeats(tournamentId) as { count: number };
      await refreshPlayers();
  ...
  async function handleRemovePlayer(playerId: number, playerName: string) {
    ...
    try {
      await api.unregisterPlayer({ tournamentId, playerId });
  ...
  const filteredRoster = roster.filter((player) => {
    ...
    return [player.name, player.email ?? '', player.phone ?? ''].join(' ').toLowerCase().includes(normalizedSearch);
  });
  ...
  function selectAllFiltered() {
    setSelectedIds((cur) => { const m = new Set(cur); filteredRoster.forEach((p) => m.add(p.id)); return Array.from(m); });
  }

  function unselectFiltered() {
    const filteredIds = new Set(filteredRoster.map((p) => p.id));
    setSelectedIds((cur) => cur.filter((id) => !filteredIds.has(id)));
  }
      ...
      <div className="space-y-3">
        ...
        {selectedTournament && (
          <div className="rounded-2xl border border-orange-700 bg-orange-950/25 px-4 py-4">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-orange-300">Setting Up Tournament</p>
            <p className="mt-1 text-base font-semibold text-orange-100">{selectedTournament.name}</p>
            <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
              <span className="rounded-full border border-orange-800 bg-orange-900/40 px-2.5 py-1 text-orange-200">Status: {selectedTournament.status}</span>
              <span className="rounded-full border border-orange-800 bg-orange-900/40 px-2.5 py-1 text-orange-200">Buy-in: ${selectedTournament.buy_in}</span>
              <span className="rounded-full border border-orange-800 bg-orange-900/40 px-2.5 py-1 text-orange-200">Bounty: ${selectedTournament.bounty_amount}</span>
            </div>
          </div>
        )}
      </div>
      ...
      <section className="space-y-3 rounded-2xl border border-gray-800 bg-gray-900/60 p-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-gray-500">Search the roster, select multiple players, then add them in one shot.</p>
          </div>
          <span className="text-xs text-gray-500 font-mono">{filteredRoster.length} available</span>
        </div>
        ...
        <div className="flex gap-3">
          ...
          <button type="button" onClick={() => { void registerPlayers(selectedPlayers); }}
            disabled={submitting || selectedPlayers.length === 0 || !tournamentId}
            className="bg-orange-500 hover:bg-orange-400 disabled:opacity-40 text-white font-semibold rounded-lg px-5 py-2 text-sm whitespace-nowrap">
            {submitting ? 'Adding…' : selectedPlayers.length > 0 ? `+ Register ${selectedPlayers.length}` : '+ Register'}
          </button>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button type="button" onClick={selectAllFiltered}
            ...
            className="rounded-md border border-gray-700 px-3 py-1.5 text-xs font-semibold text-gray-200 hover:border-orange-400 hover:text-orange-300 disabled:opacity-40">
            Select all filtered ({filteredRoster.length})
          </button>
          <button type="button" onClick={unselectFiltered} disabled={selectedInFilteredCount === 0}
            className="rounded-md border border-gray-700 px-3 py-1.5 text-xs font-semibold text-gray-200 hover:border-orange-400 hover:text-orange-300 disabled:opacity-40">
            Unselect filtered ({selectedInFilteredCount})
          </button>
          <button type="button" onClick={() => { void registerPlayers(filteredRoster); }}
            disabled={submitting || !tournamentId || filteredRoster.length === 0}
            className="rounded-md border border-emerald-700 bg-emerald-900/30 px-3 py-1.5 text-xs font-semibold text-emerald-300 hover:border-emerald-500 hover:text-emerald-200 disabled:opacity-40">
        ...
        <div className="max-h-72 overflow-y-auto rounded-xl border border-gray-800 divide-y divide-gray-800">
          {filteredRoster.length === 0 ? (
            ...
            return (
              <div key={player.id}
                className={`flex items-center justify-between gap-3 px-4 py-3 transition-colors ${selected ? 'bg-orange-950/30' : 'bg-transparent hover:bg-gray-800/70'}`}>
                <button type="button" onClick={() => toggleSelected(player.id)} className="flex flex-1 items-center gap-3 text-left">
                  <span className={`flex h-4 w-4 items-center justify-center rounded border text-[10px] font-bold ${selected ? 'border-orange-400 bg-orange-500 text-white' : 'border-gray-600 text-transparent'}`}>✓</span>
                ...
                <button type="button" onClick={() => { void registerPlayers([player]); }}
        ...
        {selectedIds.length > 0 && (
          <div className="flex items-center justify-between gap-3 rounded-lg border border-orange-900 bg-orange-950/20 px-3 py-2 text-xs text-orange-300">
            ...
            <button type="button" onClick={() => setSelectedIds([])} className="font-semibold text-orange-200 hover:text-white">Clear selection</button>
          </div>
        )}

        {roster.length === 0 && players.length === 0 && (
          <p className="text-xs text-gray-500">No players in roster yet. Add players in the <strong className="text-gray-300">Players</strong> tab first.</p>
        )}
        ...
        <div className="flex items-center gap-1 bg-gray-900 rounded-lg p-1">
          {(['list', 'tables'] as const).map((v) => (
            <button key={v} onClick={() => setView(v)}
              className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors capitalize ${view === v ? 'bg-gray-700 text-white' : 'text-gray-500 hover:text-gray-300'}`}>
              {v === 'list' ? `All Players (${players.length})` : 'Tables'}
            </button>
          ))}
        </div>
        <button onClick={() => { void handleAssignSeats(); }} disabled={assigning || players.length === 0}
          className="flex items-center gap-2 bg-violet-600 hover:bg-violet-500 disabled:opacity-40 text-white font-semibold rounded-lg px-4 py-2 text-sm whitespace-nowrap">
          <span>🎲</span>
      ...
      {view === 'list' && (
        <ul className="space-y-2">
          {players.length === 0 ? (
            ...
            return (
              <li key={p.player_id} className="flex items-center justify-between bg-gray-900 border border-gray-800 rounded-xl px-4 py-2.5">
                ...
                {selectedTournament?.status !== 'finalized' && (
                  <button type="button" onClick={() => { void handleRemovePlayer(p.player_id, p.name); }} disabled={removing}
            ...
            return (
              <div key={tableName} className={`rounded-xl border ${style.border} ${style.bg} p-4 space-y-3`}>
                <div className="flex items-center justify-between">
                  <h3 className={`font-bold text-base ${style.color}`}>{style.icon} {tableName}</h3>
                ...
                <ol className="space-y-1.5">
                  {seated.map((row) => (
                    <li key={row.id} className="flex items-center justify-between gap-3">
                      <div className="flex items-center gap-3">
                        {showSeatNumbers && (
                          <span className={`text-xs font-bold font-mono ${style.color} w-6 shrink-0`}>{row.seat_number ?? '—'}</span>
                        )}
                      ...
                      {selectedTournament?.status !== 'finalized' && (
                        <button type="button" disabled={removing}
                          onClick={() => {
                            ...
                            if (playerObj) void handleRemovePlayer(playerObj.player_id, playerObj.name);
                          }}
                  ...
                  {seated.length === 0 && <li className="text-xs text-gray-600 italic">No players assigned</li>}
                </ol>
              </div>
            );
          })}
