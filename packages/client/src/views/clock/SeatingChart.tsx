...
function TableGroup({ name, seats, keyPrefix, showSeatNumbers }: {
  name: string; seats: SeatChartEntry[]; keyPrefix: string; showSeatNumbers: boolean;
}) {
...
export default function SeatingChart({ entries, showSeatNumbers = true }: Props) {
  ...
  useEffect(() => {
    ...
    if (!viewport || !content || tables.length === 0) { setShouldScroll(false); return; }
  ...
  const content = (prefix: string) => (
    <>
      {tables.map((t) => (
        <TableGroup key={`${prefix}-${t.name}`} keyPrefix={prefix} name={t.name} seats={t.seats} showSeatNumbers={showSeatNumbers} />
      ))}
    </>
  );
