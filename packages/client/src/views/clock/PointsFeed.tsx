...
function ordinal(place: number): string {
  const r10 = place % 10, r100 = place % 100;
  if (r10 === 1 && r100 !== 11) return `${place}st`;
  if (r10 === 2 && r100 !== 12) return `${place}nd`;
  if (r10 === 3 && r100 !== 13) return `${place}rd`;
  return `${place}th`;
}

function formatTime(id: string): string {
  const epochMs = Number(id.split('-')[0]);
...
export default function PointsFeed({ entries }: Props) {
  ...
  const renderList = (prefix: string) => (
    <ul className="space-y-2 pb-1">
      {entries.map((entry) => (
        <li key={`${prefix}-${entry.id}`} className="rounded-lg border border-green-900/30 bg-black/20 px-3 py-2">
          <p className="text-[10px] text-gray-500 font-mono mb-1">{formatTime(entry.id)}</p>
